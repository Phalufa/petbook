...
import { createdTimeShort } from '../../helpers/time'
...
const Comment = ({ comment, onCommentDeleted }) => {
  const [showConfirmationBoxOnDelete, setShowConfirmationBoxOnDelete] = useState(false)
  ...
  const menu = <div className="comment-menu">
    <li className="pointer" onClick={onEdit}>Edit</li>
    <li className="pointer" onClick={onDelete}>Delete</li>
  </div>
  ...
  return (
    <div
      ...
      <div className="comment-core">
        ...
        <div className="content">{!editComment && content}
          {editComment &&
            <EditComment
              comment={comment}
