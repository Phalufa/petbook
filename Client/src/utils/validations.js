export const checkNamePattern = (errors, key, value) => {
	...
	if (!pattern.test(value)) errors[key] = 'Must contain letters only'
}

export const checkLength = (errors, key, value, length) => {
	if (value.length > length) errors[key] = 'Must be 20 characters or less'
}
...
export const years = [
	...Array(new Date().getFullYear() - 1900).keys()
].reverse()
