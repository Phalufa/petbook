...
public class AuthService {
  ...
  public void signup(SignupRequest request) throws UserAlreadyExistsException {
    ...
    User user = User.builder().username(request.getUsername()).email(request.getEmail())
        .password(encoder.encode(request.getPassword())).firstName(request.getFirstName())
        .lastName(request.getLastName()).dateOfBirth(request.getDateOfBirth()).isEnabled(false)
        .timeCreated(Instant.now()).build();

    User savedUser = userRepo.save(user);

    String verificationToken = generateToken(savedUser);
    EmailNotification notification = new EmailNotification(VERIFICATION_EMAIL_SUBJECT, user.getEmail(),
  ...
  public LoginResponse login(LoginRequest request) throws InvalidTokenException {
    ...
    return LoginResponse.builder().authenticationToken(jwtToken).username(request.getUsername())
        .refreshToken(newRefreshToken).expirationTime(Instant.now().plusMillis(provider.getExpirationTime())).build();
  }
  ...
  public LoginResponse refreshToken(RefreshTokenRequest request) throws InvalidTokenException {
    ...
    return LoginResponse.builder().authenticationToken(jwt).username(request.getUsername())
        .refreshToken(request.getToken()).expirationTime(Instant.now().plusMillis(provider.getExpirationTime()))
        .build();
  }
}